...
class ThirdStageViewController: UIViewController, subviewDelegate {
    ...
    override func viewDidLoad() {
        ...
        collisionBehavior.addBoundary(withIdentifier: "OBSTACLEBOUNDARY" as NSCopying, from: CGPoint(x: self.W*0.5, y: self.H*0.1), to: CGPoint(x: self.W*0.62, y: self.H*0.30))
                //obstacleImage.frame = CGRect(x: W*0.5, y: H*0.1, width: W*0.13, height: H*0.2)
        collisionBehavior.addBoundary(withIdentifier: "OBSTACLEBOUNDARY2" as NSCopying, from: CGPoint(x: self.W*0.5, y: self.H*0.6), to: CGPoint(x: self.W*0.62, y: self.H*0.8))
        dynamicAnimator.addBehavior(collisionBehavior)
            //obstacleImage.frame = CGRect(x: W*0.5, y: H*0.6, width: W*0.13, height: H*0.2)
    ...
    func createObstacle() {
        ...
        obstacleImage.frame = CGRect(x: W*0.5, y: H*0.1, width: W*0.12, height: H*0.2)
    ...
    func createObstacle1() {
        let obstacleImage = UIImageView(image: nil)
        obstacleImage.image = UIImage(named: "signObstacle")
        obstacleImage.frame = CGRect(x: W*0.5, y: H*0.6, width: W*0.12, height: H*0.2)
        self.view.addSubview(obstacleImage)
        self.view.bringSubviewToFront(obstacleImage)
    }
    ...
    func gameOver(){
        performSegue(withIdentifier: "passingScore3", sender: self)
    ...
    func nextStage(){
        ...
        let main = UIStoryboard(name: "Main", bundle: nil)
        let next = main.instantiateViewController(withIdentifier: "endPage")
        self.present(next, animated: true, completion: nil)
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {

            if (scoreInt >= 5){
                let saveScore1 = segue.destination as! FinishedViewController
                saveScore1.passedScore = self.ScoreLabel.text!
            }
            else
            {
